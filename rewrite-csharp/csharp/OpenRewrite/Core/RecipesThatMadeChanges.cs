...
/// <summary>
/// Marker that records which recipes made changes to a source file. Each recipe is an opaque
/// descriptor that C# keeps as raw data; the codec collapses recipes repeated within a marker
/// into a single wire-table entry referenced by the stacks.
/// </summary>
    ...
    : Marker, IRpcCodec<RecipesThatMadeChanges>, IEquatable<RecipesThatMadeChanges>
{
    ...
    public void RpcSend(RecipesThatMadeChanges after, RpcSendQueue q)
    {
        ...
        q.GetAndSendList(after, m => WireForm.From(m).RecipeTable,
