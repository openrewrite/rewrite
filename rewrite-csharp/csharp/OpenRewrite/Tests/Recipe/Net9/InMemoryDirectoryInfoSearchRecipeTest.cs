/*
 * Copyright 2026 the original author or authors.
 * <p>
 * Licensed under the Moderne Source Available License (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p>
 * https://docs.moderne.io/licensing/moderne-source-available-license
 * <p>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
using OpenRewrite.Recipes.Net9;
using OpenRewrite.Test;

namespace OpenRewrite.Tests.Recipe.Net9;

public class InMemoryDirectoryInfoSearchRecipeTest : RewriteTest
{
    [Fact]
    public void FindsInMemoryDirectoryInfoConstructor()
    {
        RewriteRun(
            spec => spec
                .SetRecipe(new InMemoryDirectoryInfoSearchRecipe()),
            CSharp(
                """
                using Microsoft.Extensions.FileSystemGlobbing;
                class Test
                {
                    void M()
                    {
                        var info = new InMemoryDirectoryInfo("/root", new[] { "/root/file.txt" });
                    }
                }
                """,
                """
                using Microsoft.Extensions.FileSystemGlobbing;
                class Test
                {
                    void M()
                    {
                        var info = /*~~(InMemoryDirectoryInfo now prepends rootDir to file paths in .NET 9. File paths that don't start with rootDir will be prefixed, which may change matching behavior.)~~>*/new InMemoryDirectoryInfo("/root", new[] { "/root/file.txt" });
                    }
                }
                """
            )
        );
    }

    [Fact]
    public void NoChangeForOtherDirectoryInfo()
    {
        RewriteRun(
            spec => spec
                .SetRecipe(new InMemoryDirectoryInfoSearchRecipe()),
            CSharp(
                """
                using System.IO;
                class Test
                {
                    void M()
                    {
                        var info = new DirectoryInfo("/root");
                    }
                }
                """
            )
        );
    }
}
