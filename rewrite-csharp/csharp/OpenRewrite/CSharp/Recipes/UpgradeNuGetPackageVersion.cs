...
public class UpgradeNuGetPackageVersion : ScanningRecipe<UpgradeNuGetPackageVersion.Accumulator>
{
    ...
    public override ITreeVisitor<ExecutionContext> GetVisitor(Accumulator acc)
    {
        return Preconditions.Check(
            new IsProjectFile(),
            new UpgradeVisitor(PackageName, acc));
    }
    ...
    private class UpgradeVisitor(string packageName, Accumulator acc)
        : XmlVisitor<ExecutionContext>
    {
        ...
        public override Xml.Xml VisitTag(Tag tag, ExecutionContext ctx)
        {
            ...
            if (t.Name == "PackageReference")
            {
                ...
                if (include != null && GlobMatcher.Matches(include, packageName))
                {
                    var targetVersion = acc.ResolvedVersions.GetValueOrDefault(include);
                    if (targetVersion != null)
                    {
            ...
            if (sourcePath != null && acc.PropertyUpdates.TryGetValue(sourcePath, out var propsForFile)
