...
/// parsed without type attestation.
/// </summary>
public class NetFrameworkAttestationTests : IDisposable
{
    ...
    public NetFrameworkAttestationTests()
    {
        ...
        // A developer machine may export FrameworkPathOverride as a workaround for exactly the
        // problem under test; clear it so this exercises what a clean build agent does.
    ...
    public async Task ProjectTargetingAnEarlierFrameworkVersionResolves()
    {
        ...
        // A sibling project on a later version, so the v4.7.2 assemblies are reachable only
        // through the fallback search paths rather than the single TargetFrameworkRootPath.
        WriteFile("newer/Newer.csproj", ClassicProject("v4.8", "Newer.cs"));
        WriteFile("newer/Newer.cs", SourceUsingFrameworkTypes("Newer"));

        // Stands in for the v4.7.2 reference assembly package so the test does not download a
        // second ~100 MB package: what is under test is the plumbing that hands MSBuild a root
        // per target framework version, not the contents of Microsoft's packages.
        Environment.SetEnvironmentVariable(
    ...
    private async Task AssertFrameworkTypesResolve(string projectFileName)
    {
        ...
        // No "type or namespace could not be found" / "predefined type not defined" errors.
    ...
    private async Task<string> LinkedReferenceAssemblies(string version)
    {
        ...
        {
            // Directory symbolic links need elevation on Windows; the assemblies this fixture
            // compiles against are few enough to copy.
