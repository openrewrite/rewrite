...
class JContainerTest {
    ...
    fun withElementsThatDoesntChangeReference() {
        val t = J.Empty(randomId(), Space.EMPTY, Markers.EMPTY)
        val trees = JContainer.build(listOf(JRightPadded(t, Space.EMPTY, Markers.EMPTY)))
        assertThat(JContainer.withElements(trees, listOf(t))).isSameAs(trees)
    }
    ...
    fun withElementsThatAddsNewElement() {
        ...
        val trees = JContainer.build(listOf(JRightPadded(t, Space.EMPTY, Markers.EMPTY)))
        assertThat(JContainer.withElements(trees, listOf(t, t2))).isNotSameAs(trees)
    }
    ...
    fun withElementsThatChangesElement() {
        val t = J.Empty(randomId(), Space.EMPTY, Markers.EMPTY)
        val trees = JContainer.build(listOf(JRightPadded(t, Space.EMPTY, Markers.EMPTY)))
        assertThat(JContainer.withElements(trees, listOf(t.withPrefix(Space.format(" ")))))
            .isNotSameAs(trees)
    }
}
