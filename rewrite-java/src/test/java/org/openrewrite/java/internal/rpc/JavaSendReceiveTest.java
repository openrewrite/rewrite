...
class JavaSendReceiveTest implements RewriteTest {
    ...
    void before() throws IOException {
        ...
        server = new RewriteRpc(new JsonRpc(new TraceMessageHandler("server", new HeaderDelimitedMessageHandler(serverIn, serverOut)), serverFormatter), env.toMarketplace(runtimeClasspath()))
          .batchSize(1);
        client = new RewriteRpc(new JsonRpc(new TraceMessageHandler("client", new HeaderDelimitedMessageHandler(clientIn, clientOut)), clientFormatter), env.toMarketplace(runtimeClasspath()))
          .batchSize(1);
    }
