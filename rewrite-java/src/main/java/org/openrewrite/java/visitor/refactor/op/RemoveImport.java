...
public class RemoveImport extends RefactorVisitor {
    ...
    private final List<J.Import> staticNamedImports = new ArrayList<>();
        ...
        if (namedImport != null && referencedTypes.stream().noneMatch(t -> t.equals(clazz))) {
            ...
            deletes.addAll(transform(getCursor().enclosingCompilationUnit(), cu -> cu
                    .withImports(cu.getImports().stream()
                            .map(i -> i == starImport ?
                                    new J.Import(randomId(), TreeBuilder.buildName(referencedTypes.iterator().next(), Formatting.format(" ")), false, i.getFormatting()) :
                                    i
                            )
                            .collect(toList())
                    )
    ...
    private List<AstTransform> delete(J.Import impoort) {
