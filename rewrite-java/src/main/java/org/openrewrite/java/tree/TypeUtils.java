...
public class TypeUtils {
    ...
    /**
     * Build a {@link TypeTree} from a type string, supporting the full grammar of a Java type reference:
     * fully-qualified or {@code java.lang} class names, primitives, arrays ({@code "int[]"}), parameterized
     * types with nested type arguments ({@code "java.util.Map<java.lang.String, java.util.List<java.lang.Integer>>"}),
     * and wildcards ({@code "?"}, {@code "? extends X"}, {@code "? super X"}).
     * <p>
     * Class names are rendered fully-qualified; callers typically follow up with
     * {@code ImportService#shortenFullyQualifiedTypeReferencesIn} to shorten them and add the corresponding imports.
     * The produced tree's {@link JavaType} is resolved with the same parser, so {@code buildTypeTree(name).getType()}
     * yields the matching type.
     */
    public static TypeTree buildTypeTree(String typeName) {
        ...
        if (genericsIndex != -1) {
            TypeTree rawType = buildTypeTree(typeName.substring(0, genericsIndex));
            List<JRightPadded<Expression>> typeParameters = new ArrayList<>();
            List<String> typeArguments = splitTypeArguments(typeName.substring(genericsIndex + 1, typeName.lastIndexOf('>')));
            for (int i = 0; i < typeArguments.size(); i++) {
                TypeTree typeParameter = buildTypeTree(typeArguments.get(i).trim());
                if (i > 0) {
                    typeParameter = typeParameter.withPrefix(Space.SINGLE_SPACE);
                }
                typeParameters.add(JRightPadded.build((Expression) typeParameter));
            }
        ...
        if (typeName.startsWith("?") && typeName.contains("extends")) {
            return new J.Wildcard(
                    ...
                    buildTypeTree(typeName.substring(typeName.indexOf("extends") + "extends".length() + 1).trim()).withPrefix(Space.SINGLE_SPACE)
            );
        }
        if (typeName.startsWith("?") && typeName.contains("super")) {
            return new J.Wildcard(
                    ...
                    buildTypeTree(typeName.substring(typeName.indexOf("super") + "super".length() + 1).trim()).withPrefix(Space.SINGLE_SPACE)
            );
        }
        if (typeName.indexOf('.') == -1) {
            ...
            if (javaLangType != null) {
                return new J.Identifier(
                        ...
                        typeName,
                        buildType(javaLangType),
                        null
                );
            }
        }
        // A direct member of `java.lang` (e.g. `java.lang.String`) is implicitly imported, so render it with
        // its simple name rather than as a fully-qualified reference that would otherwise need shortening.
        if (typeName.startsWith("java.lang.") && typeName.indexOf('.', "java.lang.".length()) == -1) {
            return new J.Identifier(
                    randomId(),
                    Space.EMPTY,
                    Markers.EMPTY,
    ...
    /**
     * Build a {@link JavaType} from a type string, supporting parameterized types with nested type arguments,
     * arrays, wildcards, primitives and fully-qualified class names. Unlike {@link JavaType#buildType(String)}
     * this understands the full grammar of a type reference (e.g.
     * {@code "java.util.Map<java.lang.String, java.util.List<java.lang.Integer>>"} or {@code "? extends java.lang.Number"}).
     */
    private static JavaType buildType(String typeName) {
        if (typeName.endsWith("]")) {
            return new JavaType.Array(null, buildType(typeName.substring(0, typeName.lastIndexOf('['))), null);
        }
        int genericsIndex = typeName.indexOf('<');
        if (genericsIndex != -1) {
            JavaType rawType = buildType(typeName.substring(0, genericsIndex));
            List<JavaType> typeParameters = new ArrayList<>();
            for (String typeParameter : splitTypeArguments(typeName.substring(genericsIndex + 1, typeName.lastIndexOf('>')))) {
                typeParameters.add(buildType(typeParameter.trim()));
            }
            return new JavaType.Parameterized(null,
                    rawType instanceof JavaType.FullyQualified ? (JavaType.FullyQualified) rawType : null, typeParameters);
        }
        if (typeName.startsWith("?")) {
            if (typeName.length() == 1) {
                return new JavaType.GenericTypeVariable(null, "?", JavaType.GenericTypeVariable.Variance.INVARIANT, (List<JavaType>) null);
            }
            int extendsIndex = typeName.indexOf("extends");
            if (extendsIndex != -1) {
                return new JavaType.GenericTypeVariable(null, "?", JavaType.GenericTypeVariable.Variance.COVARIANT,
                        Collections.singletonList(buildType(typeName.substring(extendsIndex + "extends".length()).trim())));
            }
            int superIndex = typeName.indexOf("super");
            if (superIndex != -1) {
                return new JavaType.GenericTypeVariable(null, "?", JavaType.GenericTypeVariable.Variance.CONTRAVARIANT,
                        Collections.singletonList(buildType(typeName.substring(superIndex + "super".length()).trim())));
            }
        }
        JavaType.Primitive primitive = JavaType.Primitive.fromKeyword(typeName);
        if (primitive != null) {
        ...
        return JavaType.ShallowClass.build(typeName);
    }

    /**
     * Split the comma-separated type arguments found between the angle brackets of a parameterized
     * type, respecting nesting so that arguments which are themselves parameterized are not split on
     * their inner commas (e.g. {@code "String, Map<String, Integer>"} yields {@code ["String", " Map<String, Integer>"]}).
     */
    private static List<String> splitTypeArguments(String typeArguments) {
        List<String> result = new ArrayList<>();
        int depth = 0;
        int start = 0;
        for (int i = 0; i < typeArguments.length(); i++) {
            char c = typeArguments.charAt(i);
            if (c == '<') {
                depth++;
            } else if (c == '>') {
                depth--;
