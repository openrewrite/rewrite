...
public class TemplateMatcher {
    ...
    private final TemplateMatcherVisitor visitor = new TemplateMatcherVisitor();
    private final J template;

    public TemplateMatcher(J template) {
        this.template = template;
    }

    public boolean matches(J tree) {
        visitor.reset();
        visitor.visit(template, tree);
        return visitor.isEqual.get();
    }

    public J getParameter(int i) {
        return visitor.extractedParameters.get(i);
    }

    @SuppressWarnings("ConstantConditions")
    private static class TemplateMatcherVisitor extends JavaIsoVisitor<J> {
        AtomicBoolean isEqual = new AtomicBoolean(true);
        List<J> extractedParameters = new ArrayList<>();
        ...
        private boolean isTemplateParameter(J.MethodInvocation method) {
            return method.getSelect() instanceof J.Identifier && ((J.Identifier) method.getSelect()).getSimpleName().equals("__P__");
        }

        private boolean matchTemplateParameter(J.MethodInvocation method, J j) {
            return j instanceof TypedTree && TypeUtils.isOfType(method.getType(), ((TypedTree) j).getType()) && extractedParameters.add(j);
        }

        private void addExtractedParameter(J j) {
            extractedParameters.add(j);
        }
        ...
        public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, J j) {
            if (isEqual.get()) {
                if (isTemplateParameter(method)) {
                    if (matchTemplateParameter(method, j)) {
                        addExtractedParameter(j);
                        return method;
                    } else {
