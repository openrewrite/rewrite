...
/**
 * Abstract base class for scanning recipes that delegate all their behavior to another scanning recipe.
 * @param <T> the type of the accumulator populated within the scanning phase.
 */
public abstract class DecoratedScanningRecipe<T> extends ScanningRecipe<T> implements Recipe.DelegatingRecipe {

    /**
     * @return the scanning recipe to delegate all behavior to
     */
    public abstract ScanningRecipe<T> getDelegate();

    @Override
    public T getInitialValue(ExecutionContext ctx) {
        return getDelegate().getInitialValue(ctx);
    }

    @Override
    public TreeVisitor<?, ExecutionContext> getScanner(T acc) {
        return getDelegate().getScanner(acc);
    }

    @Override
    public TreeVisitor<?, ExecutionContext> getVisitor(T acc) {
        return getDelegate().getVisitor(acc);
    }

    @Override
    public Collection<? extends SourceFile> generate(T acc, ExecutionContext ctx) {
        return getDelegate().generate(acc, ctx);
    }
