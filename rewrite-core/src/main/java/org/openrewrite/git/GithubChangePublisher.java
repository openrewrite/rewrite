...
public class GithubChangePublisher implements ChangePublisher {
    ...
    public void publish(Collection<Result> results) {
        results.forEach(this::publishEach);
    }

    public void publishEach(Result result) {
        Timer.Sample sample = Timer.start();
        if (result.getAfter() == null) {
            // FIXME delete file
        } else {
            result.getAfter().getMarkers().findFirst(Github.class).ifPresent(metadata -> {
                ...
                Timer.Builder timer = Timer.builder("rewrite.publish.github")
                        ...
                        .tag("tree.type", result.getAfter().getClass().getSimpleName());

                try {
                    GHRepository ghRepo = github.getRepository(organization + "/" + repository);

                    try {
                        GHContent fileContent = ghRepo.getFileContent(result.getAfter().getSourcePath().toString());

                        if (verifyOriginal && result.getBefore() != null) {
                            try (Scanner scanner = new Scanner(fileContent.read(), StandardCharsets.UTF_8.name())) {
                                ...
                                if (!result.getBefore().print().equals(fileContentString)) {
                                    logger.warn("Attempting to make a change to " + organization +
                                            "/" + repository + ":" + result.getBefore().getSourcePath() +
                                    ...
                                    logger.info("Change is already present in " + organization + "/" + repository + ":" + result.getAfter().getSourcePath() + " in GitHub");
                        ...
                        fileContent.update(result.getAfter().print(), commitMessage);
                        logger.info("Published change " + organization + "/" + repository + ":" + result.getAfter().getSourcePath() + " to GitHub");
                        ...
                        ghRepo.createContent()
                                .branch("master")
                                .path(result.getAfter().getSourcePath().toString())
                                .content(result.getAfter().print())
