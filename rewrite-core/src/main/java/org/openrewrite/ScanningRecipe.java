...
public abstract class ScanningRecipe<T> extends Recipe {
     ...
     *
     * @param acc                  The accumulated scanning data.
     * @param generatedInThisStage Files generated by other recipes in this stage.
     * @return A list of new source files.
     */
    public Collection<? extends SourceFile> generate(T acc, Collection<SourceFile> generatedInThisStage, ExecutionContext ctx) {
    ...
    /**
     * Contribute recipes to a subsequent <em>stage</em>, chosen dynamically from what scanning found, using
     * the now-populated accumulator. Any recipe added to {@code stage} runs in a fresh downstream stage with
     * its own scan/generate/edit lifecycle over the current sources. Adding at least one recipe schedules that
     * stage to run.
     *
     * @param stage The recipe list to append downstream-stage recipes to.
     * @param acc  The accumulated scanning data.
     */
    @Incubating(since = "8.87.0")
    public void nextStage(RecipeList stage, ExecutionContext ctx, T acc) {
    }

    @Override
    public final void nextStage(RecipeList next, ExecutionContext ctx) {
        super.nextStage(next, ctx);
        //noinspection unchecked
        T acc = (T) ctx.getStageDetails().getRootCursor().getRoot().getMessage(getRecipeAccMessage());
        if (acc != null) {
            nextStage(next, ctx, acc);
        }
    }
