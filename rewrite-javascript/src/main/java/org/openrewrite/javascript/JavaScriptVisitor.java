...
public class JavaScriptVisitor<P> extends JavaVisitor<P> {
    ...
    public J visitImportTypeAttributes(JS.ImportTypeAttributes importTypeAttributes, P p) {
        ...
        i = i.withEnd(requireNonNull(visitSpace(i.getEnd(), JsSpace.Location.IMPORT_TYPE_ATTRIBUTES_END_SUFFIX, p)));
        return i;
    }
    ...
    public J visitImportAttribute(JS.ImportAttribute importAttribute, P p) {
        ...
        i = i.getPadding().withValue(requireNonNull(visitLeftPadded(i.getPadding().getValue(), JsLeftPadded.Location.IMPORT_ATTRIBUTE_VALUE, p)));
        return i;
    }
    ...
    public J visitComputedPropertyName(JS.ComputedPropertyName computedPropertyName, P p) {
        ...
        t = t.getPadding().withExpression(requireNonNull(visitRightPadded(t.getPadding().getExpression(), JsRightPadded.Location.COMPUTED_PROPERTY_NAME_SUFFIX, p)));
        return t;
    }
    ...
    public J visitJsxTag(JSX.Tag tag, P p) {
        ...
        if (t.isSelfClosing()) {
            t = t.withSelfClosing(visitSpace(t.getSelfClosing(), Space.Location.LANGUAGE_EXTENSION, p));
