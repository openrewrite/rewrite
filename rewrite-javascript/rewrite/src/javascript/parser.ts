...
import {emptySpace, JavaType, J, Statement} from '../java';
import {JS, TypedTree, TypeTree, Expression} from './tree';
import {
    emptyMarkers,
    ExecutionContext,
    Markers,
...
export class JavaScriptParserVisitor {
    ...
    visitSourceFile(node: ts.SourceFile): JS.CompilationUnit {
        ...
        let prefix: J.Space = this.prefix(node);
    ...
    // private rightPaddedList<N extends ts.Node, T extends J.J>(nodes: ts.NodeArray<N>, trailing: (node: N) => Space, markers?: (node: N) => Markers): J.RightPadded<T>[] {
    ...
    private keywordPrefix = (token: ts.PunctuationSyntaxKind, findSibling: (node: ts.Node) => ts.Node | null) => (node: ts.Node): J.Space => {
    ...
    private mapExtends(node: ts.ClassDeclaration | ts.ClassExpression): J.LeftPadded<TypeTree> | null {
    ...
    private mapInterfaceExtends(node: ts.InterfaceDeclaration): J.Container<J.TypeTree> | null {
        if (node.heritageClauses == undefined || node.heritageClauses.length == 0) {
            return null;
        }
        for (let heritageClause of node.heritageClauses) {
            if ((heritageClause.token == ts.SyntaxKind.ExtendsKeyword)) {
                const _extends: J.RightPadded<J.TypeTree>[] = [];
    ...
    private mapImplements(node: ts.ClassDeclaration | ts.ClassExpression): J.Container<J.TypeTree> | null {
        if (node.heritageClauses == undefined || node.heritageClauses.length == 0) {
            return null;
        }
        for (let heritageClause of node.heritageClauses) {
    ...
    visitConstructorType(node: ts.ConstructorTypeNode) {
        return {
            ...
            parameters: {
                kind: J.Kind.JContainer,
                prefix: this.prefix(node.getChildAt(node.getChildren().findIndex(n => n.pos === node.parameters.pos) - 1)),
                markers: emptyMarkers,
                elements:
                    node.parameters.length == 0 ?
                        [this.rightPadded(this.newJEmpty(), this.prefix(this.findChildNode(node, ts.SyntaxKind.CloseParenToken)!))]
    ...
    visitTypeQuery(node: ts.TypeQueryNode) {
        return {
            ...
            typeArguments: node.typeArguments ? this.mapTypeArguments(this.suffix(node.exprName), node.typeArguments) : null,
            type: this.mapType(node)
        }
    }
        ...
        return {
            ...
            members: {
                ...
                statements: node.members.map(te => ({
                    ...
                    suffix: (te.getLastToken()?.kind === ts.SyntaxKind.SemicolonToken) || (te.getLastToken()?.kind === ts.SyntaxKind.CommaToken) ? this.prefix(te.getLastToken()!) : emptySpace,
                    markers: (te.getLastToken()?.kind === ts.SyntaxKind.SemicolonToken) || (te.getLastToken()?.kind === ts.SyntaxKind.CommaToken) ? Markers.build([this.convertToken(te.getLastToken())!]) : emptyMarkers
                })),
                end: this.prefix(node.getLastToken()!)
            },
            type: this.mapType(node)
    ...
    visitArrayType(node: ts.ArrayTypeNode) {
        return {
            ...
            // TODO what is the remainder of this?
            // null,
    ...
    visitConditionalType(node: ts.ConditionalTypeNode) {
        return {
            ...
            condition: {
                kind: J.Kind.JContainer,
                prefix: this.prefix(this.findChildNode(node, ts.SyntaxKind.ExtendsKeyword)!),
    ...
    visitInferType(node: ts.InferTypeNode) {
        return {
            ...
            prefix: this.prefix(node),
            markers: emptyMarkers,
            typeParameters: this.leftPadded(emptySpace, this.convert(node.typeParameter)),
            type: this.mapType(node)
        };
    }
    ...
    visitMappedType(node: ts.MappedTypeNode) {
        ...
        //         keysRemapping: new JS.MappedType.KeysRemapping(
        //             randomId(),
        //             this.prefix(this.findChildNode(node, ts.SyntaxKind.OpenBracketToken)!),
        //             emptyMarkers,
        //             this.rightPadded(
        ...
        //         valueType: node.type ? new J.Container(
    ...
    visitBindingElement(node: ts.BindingElement) {
        return new JS.BindingElement(
            ...
            node.initializer ? this.leftPadded(this.prefix(this.findChildNode(node, ts.SyntaxKind.EqualsToken)!), this.convert<Expression>(node.initializer)) : null,
            this.mapVariableType(node),
        );
    }
    ...
    private convertPropertyAssignments(nodes: ts.Node[]): J.Block {
        ...
        const statements: J.RightPadded<J.Statement>[] = this.rightPaddedSeparatedList(
    ...
    visitCallExpression(node: ts.CallExpression) {
        ...
        let select: J.RightPadded<Expression> | null;
    ...
    visitNewExpression(node: ts.NewExpression) {
        return new J.NewClass(
            ...
            node.arguments ? this.mapCommaSeparatedList(this.getParameterListNodes(node)) : J.Container.empty<Expression>().withMarkers(Markers.build([(new J.OmitParentheses(randomId()))])),
            null,
            this.mapMethodType(node)
        );
    }
    ...
    visitClassExpression(node: ts.ClassExpression) {
        return new JS.StatementExpression(
            ...
            new J.ClassDeclaration(
                ...
                this.mapTypeParametersAsContainer(node),
                ...
                new J.Block(
                    ...
                    emptyMarkers,
                    this.rightPadded(false, emptySpace),
                    node.members.map(ce => new J.RightPadded(
                        this.convert(ce),
                        ce.getLastToken()?.kind === ts.SyntaxKind.SemicolonToken ? this.prefix(ce.getLastToken()!) : emptySpace,
                        ce.getLastToken()?.kind === ts.SyntaxKind.SemicolonToken ? Markers.build([new Semicolon(randomId())]) : emptyMarkers
                    )),
    ...
    visitReturnStatement(node: ts.ReturnStatement) {
        return new J.Return(
            ...
            this.prefix(node),
            emptyMarkers,
            node.expression ? this.convert<Expression>(node.expression) : null
        );
    }
    ...
    visitInterfaceDeclaration(node: ts.InterfaceDeclaration) {
        return new J.ClassDeclaration(
            ...
            new J.Block(
                ...
                emptyMarkers,
                this.rightPadded(false, emptySpace),
                node.members.map(te => new J.RightPadded(
                    this.convert(te),
    ...
    private mapCommaSeparatedList<T extends J.J>(nodes: readonly ts.Node[]): J.Container<T> {
        return this.mapToContainer(nodes, this.trailingComma(nodes));
    }

    private mapTypeArguments(prefix: Space, nodes: readonly ts.Node[]): J.Container<Expression> {
        if (nodes.length === 0) {
            return J.Container.empty();
        }
    ...
    private mapToContainer<T>(nodes: readonly ts.Node[], markers?: (ns: readonly ts.Node[], i: number) => Markers): J.Container<T> {
        if (nodes.length === 0) {
            return J.Container.empty();
        }
        const prefix = this.prefix(nodes[0]);
    ...
    private mapToRightPaddedList<T>(node: ts.SyntaxList, lastAfter: J.Space, markers?: (ns: readonly ts.Node[], i: number) => Markers): J.RightPadded<T>[] {
    ...
    private mapTypeParametersAsContainer(node: ts.ClassDeclaration | ts.InterfaceDeclaration | ts.ClassExpression): J.Container<J.TypeParameter> | null {
