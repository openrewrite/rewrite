...
function analyzeClass(content: string, queueType: 'RpcSendQueue' | 'RpcReceiveQueue', extractor: PropertyExtractor): Map<string, string[]> {
    ...
    // JavaScript uses 'override async visit...' (with or without 'Js' prefix)
    const terminatorPattern = /(?=override async visit|public override async visit)/;

    const methodPattern = new RegExp(
        `override async visit(?:Js)?(\\w+)\\([^:]+:\\s*JS\\.(\\S+?),\\s*q:\\s*${queueType}\\)[^{]*\\{([\\s\\S]*?)${terminatorPattern.source}`,
        'g'
    );
...
function generateReport() {
    ...
    // Split content into JavaScriptSender and JavaScriptReceiver sections
    const senderMatch = content.match(/class JavaScriptSender[\s\S]*?(?=class JavaScriptDelegateSender)/);
    const receiverMatch = content.match(/class JavaScriptReceiver[\s\S]*?(?=class JavaScriptDelegateReceiver)/);

    if (!senderMatch || !receiverMatch) {
        console.error('Could not find JavaScriptSender or JavaScriptReceiver classes');
        return;
    }
    ...
    // Build JavaScriptSender output
    let senderOutput = 'JavaScriptSender\n';
    ...
    // Build JavaScriptReceiver output
    let receiverOutput = 'JavaScriptReceiver\n';
