...
/**
 * Where `process.cwd()` resolves to for a template configured with these declarations. `process` is
 * a global `@types/node` declares, reachable only through automatic type inclusion — unlike a module
 * specifier, which resolves by path whatever this option says.
 */
async function processDeclaringType(
    ...
    // `name` is the callee's own spelling whether or not anything resolved, so the declaring type
    // is what says the declarations were loaded.
