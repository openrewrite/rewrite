...
describe('WhitespaceReconciler', () => {
    ...
    const parseNoTypes = (code: string) =>
        parserNoTypes.parseOne({sourcePath: 'test.ts', text: code}) as Promise<JS.CompilationUnit>;
    ...
    it('skipTypes option should produce AST without types', async () => {
        const withTypes = await parse('const x: number = 1');
        const withoutTypes = await parseNoTypes('const x: number = 1');
    ...
    it('should reconcile multi-line formatting', async () => {
        ...
        const formatted = await parseNoTypes(`const fn = () => {
  return 1;
};
`);
