...
class TomlTableValueTest {
    ...
    @Test
    void preservesMultilineDelimiterAndEscapesBasicStrings() {
        Toml.Document document = document("library = { name = \"\"\"old\"\"\" }\n");
        Toml.Table table = table(document);

        Toml.Table updated = TomlTableValue.withString(table, "name", "new\\value");

        assertThat(literal(updated, "name").getSource()).isEqualTo("\"\"\"new\\\\value\"\"\"");
        assertThat(print(withTableValue(document, updated)))
                .isEqualTo("library = { name = \"\"\"new\\\\value\"\"\" }\n");
    }
