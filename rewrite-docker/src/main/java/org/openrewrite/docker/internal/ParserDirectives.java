 ...
 * directive as it reads, so a directive is in the tree only as the comment it is written as.
 */
public class ParserDirectives {
    ...
    /// A backslash unless an `escape` directive names the backtick. Docker takes only those two, so a
    /// value that is neither leaves the file on the default.
    ...
    /// A directive is written in the prefix of whatever the file opens with, not in
    /// [Docker.File#getPrefix()], which is empty.
    ...
    /// Directives run from the first line with nothing between them, which is what tells one from a
    /// comment written the same way. Anything else ends the head of the file for good.
