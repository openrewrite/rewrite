/*
 * Copyright 2022 the original author or authors.
 * <p>
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p>
 * https://www.apache.org/licenses/LICENSE-2.0
 * <p>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.openrewrite.xml.security;

import org.junit.jupiter.api.Test;
import org.openrewrite.test.AdHocRecipe;
import org.openrewrite.test.RecipeSpec;
import org.openrewrite.test.RewriteTest;

import static org.openrewrite.xml.Assertions.xml;

class HasOwaspSuppressionsFileTest implements RewriteTest {

    @Override
    public void defaults(RecipeSpec spec) {
        spec.recipe(new AdHocRecipe(HasOwaspSuppressionsFile::new));
    }

    @Test
    void noChangesIfNoSuppressionsFile() {
        rewriteRun(
                xml("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n" +
                                "<document/>",
                        spec -> spec.path("soppressata.xml")
                )
        );
    }

    @Test
    void changesIfSuppressionsFile() {
        rewriteRun(
                xml("""
                        <?xml version="1.0" encoding="UTF-8" ?>
                        <document/>""",
                        """
                        <!--~~(Found it)~~>--><?xml version="1.0" encoding="UTF-8" ?>
                        <document/>""",
                        spec -> spec.path("suppressions.xml")
                )
        );
    }

    @Test
    void doesntChangeIfNotAtRoot() {
        rewriteRun(
                xml("""
                        <?xml version="1.0" encoding="UTF-8" ?>
                        <document/>""",
                        spec -> spec.path("not/root/suppressions.xml")
                )
        );
    }

    @Test
    void onlyChangesRoot() {
        rewriteRun(
                xml("""
                        <?xml version="1.0" encoding="UTF-8" ?>
                        <document/>""",
                        spec -> spec.path("not/root/suppressions.xml")
                ),
                xml("""
                        <?xml version="1.0" encoding="UTF-8" ?>
                        <document/>""",
                        """
                        <!--~~(Found it)~~>--><?xml version="1.0" encoding="UTF-8" ?>
                        <document/>""",
                        spec -> spec.path("suppressions.xml")
                )
        );
    }
}
