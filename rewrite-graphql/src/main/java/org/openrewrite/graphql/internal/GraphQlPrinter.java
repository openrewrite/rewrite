...
public class GraphQlPrinter<P> extends GraphQlVisitor<PrintOutputCapture<P>> {
    ...
    public GraphQl visitArguments(GraphQl.Arguments arguments, PrintOutputCapture<P> p) {
        ...
        p.append("(");
        List<GraphQl.Argument> args = arguments.getArguments();
        for (int i = 0; i < args.size(); i++) {
            GraphQl.Argument arg = args.get(i);
            visitArgument(arg, p);
            
            // Check if we need to add a comma
            if (i < args.size() - 1 && shouldAddCommaBetween(args.get(i), args.get(i + 1))) {
                p.append(",");
                String nextPrefix = args.get(i + 1).getPrefix().getWhitespace();
                if (!nextPrefix.isEmpty() && !nextPrefix.startsWith(" ") && !nextPrefix.startsWith("\n")) {
                    p.append(" ");
                }
            }
        }
    ...
    public GraphQl visitListValue(GraphQl.ListValue listValue, PrintOutputCapture<P> p) {
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            if (i < values.size() - 1 && shouldAddCommaBetween(values.get(i), values.get(i + 1))) {
                p.append(",");
                String nextPrefix = values.get(i + 1).getPrefix().getWhitespace();
                if (!nextPrefix.isEmpty() && !nextPrefix.startsWith(" ") && !nextPrefix.startsWith("\n")) {
                    p.append(" ");
                }
            }
    ...
    public GraphQl visitObjectValue(GraphQl.ObjectValue objectValue, PrintOutputCapture<P> p) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if (i < fields.size() - 1 && shouldAddCommaBetween(fields.get(i), fields.get(i + 1))) {
                p.append(",");
                String nextPrefix = fields.get(i + 1).getPrefix().getWhitespace();
                if (!nextPrefix.isEmpty() && !nextPrefix.startsWith(" ") && !nextPrefix.startsWith("\n")) {
                    p.append(" ");
                }
            }
    ...
    public GraphQl visitSchemaDefinition(GraphQl.SchemaDefinition schemaDef, PrintOutputCapture<P> p) {
        ...
        if (schemaDef.getDescription() != null) {
            visitStringValue(schemaDef.getDescription(), p);
            p.append(" ");
        }
        p.append("schema");
    ...
    public GraphQl visitScalarTypeDefinition(GraphQl.ScalarTypeDefinition scalarType, PrintOutputCapture<P> p) {
        ...
        if (scalarType.getDescription() != null) {
            visitStringValue(scalarType.getDescription(), p);
            p.append(" ");
        }
        p.append("scalar");
    ...
    public GraphQl visitObjectTypeDefinition(GraphQl.ObjectTypeDefinition objectType, PrintOutputCapture<P> p) {
        ...
        if (objectType.getDescription() != null) {
            visitStringValue(objectType.getDescription(), p);
            p.append(" ");
        }
        p.append("type");
        visitName(objectType.getName(), p);
        visitImplementsInterfaces(objectType.getImplementsInterfaces(), p);
    ...
    public GraphQl visitFieldDefinition(GraphQl.FieldDefinition fieldDef, PrintOutputCapture<P> p) {
        ...
        if (fieldDef.getDescription() != null) {
            visitStringValue(fieldDef.getDescription(), p);
            p.append(" ");
        }
        visitName(fieldDef.getName(), p);
        visitInputValueDefinitions(fieldDef.getArguments(), fieldDef.getArgumentsEnd(), p);
        p.append(":");
    ...
    public GraphQl visitInputValueDefinition(GraphQl.InputValueDefinition inputValueDef, PrintOutputCapture<P> p) {
        ...
        if (inputValueDef.getDescription() != null) {
            visitStringValue(inputValueDef.getDescription(), p);
            p.append(" ");
        }
    ...
    public GraphQl visitInterfaceTypeDefinition(GraphQl.InterfaceTypeDefinition interfaceType, PrintOutputCapture<P> p) {
        ...
        if (interfaceType.getDescription() != null) {
            visitStringValue(interfaceType.getDescription(), p);
            p.append(" ");
        }
        p.append("interface");
        visitName(interfaceType.getName(), p);
        visitImplementsInterfaces(interfaceType.getImplementsInterfaces(), p);
    ...
    public GraphQl visitUnionTypeDefinition(GraphQl.UnionTypeDefinition unionType, PrintOutputCapture<P> p) {
        ...
        if (unionType.getDescription() != null) {
            visitStringValue(unionType.getDescription(), p);
            p.append(" ");
        }
        p.append("union");
        visitName(unionType.getName(), p);
        visitList(unionType.getDirectives(), p);
        if (unionType.getMemberTypes() != null && !unionType.getMemberTypes().isEmpty()) {
            p.append(" =");
            for (int i = 0; i < unionType.getMemberTypes().size(); i++) {
                if (i > 0) {
                    p.append(" |");
                }
                visitNamedType(unionType.getMemberTypes().get(i), p);
            }
        }
    ...
    public GraphQl visitEnumTypeDefinition(GraphQl.EnumTypeDefinition enumType, PrintOutputCapture<P> p) {
        ...
        if (enumType.getDescription() != null) {
            visitStringValue(enumType.getDescription(), p);
            p.append(" ");
        }
        p.append("enum");
    ...
    public GraphQl visitEnumValueDefinition(GraphQl.EnumValueDefinition enumValueDef, PrintOutputCapture<P> p) {
        ...
        if (enumValueDef.getDescription() != null) {
            visitStringValue(enumValueDef.getDescription(), p);
            p.append(" ");
        }
    ...
    public GraphQl visitInputObjectTypeDefinition(GraphQl.InputObjectTypeDefinition inputObjectType, PrintOutputCapture<P> p) {
        ...
        if (inputObjectType.getDescription() != null) {
            visitStringValue(inputObjectType.getDescription(), p);
            p.append(" ");
        }
        p.append("input");
    ...
    public GraphQl visitDirectiveDefinition(GraphQl.DirectiveDefinition directiveDef, PrintOutputCapture<P> p) {
        ...
        if (directiveDef.getDescription() != null) {
            visitStringValue(directiveDef.getDescription(), p);
            p.append(" ");
        }
        p.append("directive @");
        ...
        for (int i = 0; i < directiveDef.getLocations().size(); i++) {
            ...
            // For locations after the first, print the pipe before the prefix
            // The parser consumes pipes with sourceBefore("|"), so we need to re-add them
            if (i > 0) {
                p.append(" |");
                // Print the prefix of the location (whitespace after the pipe)
                printSpace(location.getElement().getPrefix(), p);
            } else {
                // For the first location, just print its prefix
                printSpace(location.getElement().getPrefix(), p);
            }
    ...
    public GraphQl visitObjectTypeExtension(GraphQl.ObjectTypeExtension objectExt, PrintOutputCapture<P> p) {
        ...
        p.append("extend type");
        visitName(objectExt.getName(), p);
        visitImplementsInterfaces(objectExt.getImplementsInterfaces(), p);
    ...
    public GraphQl visitInterfaceTypeExtension(GraphQl.InterfaceTypeExtension interfaceExt, PrintOutputCapture<P> p) {
        ...
        p.append("extend interface");
        visitName(interfaceExt.getName(), p);
        visitImplementsInterfaces(interfaceExt.getImplementsInterfaces(), p);
    ...
    public GraphQl visitUnionTypeExtension(GraphQl.UnionTypeExtension unionExt, PrintOutputCapture<P> p) {
        ...
        p.append("extend union");
        visitName(unionExt.getName(), p);
        visitList(unionExt.getDirectives(), p);
        if (unionExt.getMemberTypes() != null && !unionExt.getMemberTypes().isEmpty()) {
            p.append(" =");
            for (int i = 0; i < unionExt.getMemberTypes().size(); i++) {
                if (i > 0) {
                    p.append(" |");
                }
                visitNamedType(unionExt.getMemberTypes().get(i), p);
            }
        }
    ...
    private void visitImplementsInterfaces(@Nullable List<GraphQl.NamedType> interfaces, PrintOutputCapture<P> p) {
        if (interfaces != null && !interfaces.isEmpty()) {
            ...
            for (int i = 0; i < interfaces.size(); i++) {
                ...
                visitNamedType(interfaces.get(i), p);
            }
        }
    }
    ...
    private void visitVariableDefinitions(@Nullable List<GraphQl.VariableDefinition> varDefs, @Nullable Space end, PrintOutputCapture<P> p) {
        if (varDefs != null && !varDefs.isEmpty()) {
            ...
            for (int i = 0; i < varDefs.size(); i++) {
                ...
                if (i < varDefs.size() - 1 && shouldAddCommaBetween(varDef, varDefs.get(i + 1))) {
                    p.append(",");
                    String nextPrefix = varDefs.get(i + 1).getPrefix().getWhitespace();
                    if (!nextPrefix.isEmpty() && !nextPrefix.startsWith(" ") && !nextPrefix.startsWith("\n")) {
                        p.append(" ");
                    }
                }
    ...
    private void visitInputValueDefinitions(@Nullable List<GraphQl.InputValueDefinition> inputDefs, @Nullable Space end, PrintOutputCapture<P> p) {
        if (inputDefs != null && !inputDefs.isEmpty()) {
            ...
            for (int i = 0; i < inputDefs.size(); i++) {
                ...
                // Input value definitions in schema don't use commas, they're whitespace-separated
                // The parser preserves the original formatting, so we don't add commas
            }
            printSpace(end, p);
            p.append(")");
        }
    ...
    private boolean shouldAddCommaBetween(GraphQl first, GraphQl second) {
        String nextPrefix = second.getPrefix().getWhitespace();
        // Don't add comma if comma already exists in whitespace
        return !nextPrefix.contains(",");
    }
