...
public class FindTypeUsages extends Recipe {
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new GraphQlIsoVisitor<ExecutionContext>() {
            ...
            @Override
            public GraphQl.InlineFragment visitInlineFragment(GraphQl.InlineFragment inlineFragment, ExecutionContext ctx) {
                GraphQl.InlineFragment i = inlineFragment;
                
                // Manually handle the type condition since automatic visitor dispatch isn't working
                if (i.getTypeCondition() != null) {
                    GraphQl.NamedType typeCondition = i.getTypeCondition();
                    if (typeCondition.getName().getValue().equals(typeName)) {
                        typeCondition = SearchResult.found(typeCondition);
                    }
                    // Also need to visit the name within the type condition
                    typeCondition = typeCondition.withName(visitName(typeCondition.getName(), ctx));
                    i = i.withTypeCondition(typeCondition);
                }
                
                // Visit directives
                if (i.getDirectives() != null) {
                    i = i.withDirectives(ListUtils.map(i.getDirectives(), 
                        d -> (GraphQl.Directive) visit(d, ctx)));
                }
                
                // Visit selection set
                i = i.withSelectionSet((GraphQl.SelectionSet) visit(i.getSelectionSet(), ctx));
