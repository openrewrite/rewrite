...
class CustomResolveVisitor extends ResolveVisitor {
    ...
    private Map<GenericsType.GenericsTypeName, GenericsType> genericParameterNames = new HashMap<>();
    ...
    public CustomResolveVisitor(CompilationUnit compilationUnit) {
    ...
    public void visitField(final FieldNode node) {
        Map<GenericsType.GenericsTypeName, GenericsType> oldNames = genericParameterNames;
    ...
    private boolean resolveToOuterNested(final ClassNode type) {
        ...
        BiConsumer<CompileUnit.ConstructedOuterNestedClassNode, ClassNode> setRedirectListener = (s, c) -> type.setRedirect(s);
        ...
        for (ImportNode importNode : module.getStaticImports().values()) {
            ...
            CompileUnit.ConstructedOuterNestedClassNode constructedOuterNestedClassNode = tryToConstructOuterNestedClassNodeViaStaticImport(compileUnit, importNode, importFieldName, setRedirectListener);
        ...
        for (Map.Entry<String, ClassNode> entry : compileUnit.getClassesToCompile().entrySet()) {
            ...
            CompileUnit.ConstructedOuterNestedClassNode constructedOuterNestedClassNode = tryToConstructOuterNestedClassNode(type, outerClassNode, setRedirectListener);
            if (null != constructedOuterNestedClassNode) {
                compileUnit.addClassNodeToResolve(constructedOuterNestedClassNode);
                return true;
            }
        ...
        for (ImportNode importNode : module.getStaticStarImports().values()) {
            CompileUnit.ConstructedOuterNestedClassNode constructedOuterNestedClassNode = tryToConstructOuterNestedClassNodeViaStaticImport(compileUnit, importNode, typeName, setRedirectListener);
            if (null != constructedOuterNestedClassNode) {
                compileUnit.addClassNodeToResolve(constructedOuterNestedClassNode);
        ...
        if (typeName.indexOf('.') == -1) {
            ...
            for (ClassNode cn : hierClasses.values()) {
                CompileUnit.ConstructedOuterNestedClassNode constructedOuterNestedClassNode = tryToConstructOuterNestedClassNodeForBaseType(compileUnit, typeName, cn, setRedirectListener);
                if (null != constructedOuterNestedClassNode) {
                    compileUnit.addClassNodeToResolve(constructedOuterNestedClassNode);
    ...
    private CompileUnit.ConstructedOuterNestedClassNode tryToConstructOuterNestedClassNodeViaStaticImport(final CompileUnit compileUnit, final ImportNode importNode, final String typeName, final BiConsumer<CompileUnit.ConstructedOuterNestedClassNode, ClassNode> setRedirectListener) {
        ...
        CompileUnit.ConstructedOuterNestedClassNode constructedOuterNestedClassNode = new CompileUnit.ConstructedOuterNestedClassNode(outerClassNode, outerNestedClassName);
        constructedOuterNestedClassNode.addSetRedirectListener(setRedirectListener);
        return constructedOuterNestedClassNode;
    }

    private CompileUnit.ConstructedOuterNestedClassNode tryToConstructOuterNestedClassNode(final ClassNode type, final ClassNode outerClassNode, final BiConsumer<CompileUnit.ConstructedOuterNestedClassNode, ClassNode> setRedirectListener) {
    ...
    private CompileUnit.ConstructedOuterNestedClassNode tryToConstructOuterNestedClassNodeForBaseType(final CompileUnit compileUnit, final String typeName, final ClassNode cn, final BiConsumer<CompileUnit.ConstructedOuterNestedClassNode, ClassNode> setRedirectListener) {
        if (!compileUnit.getClassesToCompile().containsValue(cn)) return null;

        String outerNestedClassName = cn.getName() + "$" + typeName;
        CompileUnit.ConstructedOuterNestedClassNode constructedOuterNestedClassNode = new CompileUnit.ConstructedOuterNestedClassNode(cn, outerNestedClassName);
        constructedOuterNestedClassNode.addSetRedirectListener(setRedirectListener);
        return constructedOuterNestedClassNode;
    }
    ...
    protected boolean resolve(final ClassNode type, final boolean testModuleImports, final boolean testDefaultImports, final boolean testStaticInnerClasses) {
        ...
        GenericsType typeParameter = genericParameterNames.get(new GenericsType.GenericsTypeName(typeName));
...
//            return ExpressionUtils.transformInlineConstants(exp);
    ...
    private void resolveOuterNestedClassFurther(final ClassNode node) {
        ...
        Map<String, CompileUnit.ConstructedOuterNestedClassNode> classesToResolve = compileUnit.getClassesToResolve();
        List<String> resolvedInnerClassNameList = new LinkedList<>();

        for (Map.Entry<String, CompileUnit.ConstructedOuterNestedClassNode> entry : classesToResolve.entrySet()) {
            String innerClassName = entry.getKey();
            CompileUnit.ConstructedOuterNestedClassNode constructedOuterNestedClass = entry.getValue();
