...
class GroovyTypeMapping implements JavaTypeMapping<ASTNode> {
    ...
    GroovyTypeMapping(JavaTypeFactory typeFactory) {
    ...
    private JavaType.FullyQualified classType(ClassNode node, String signature) {
        try {
            ...
            return typeFactory.computeClass(signature, node.getName(), Flag.Public.getBitMask(),
                ...
                stub.unsafeSet(null, supertype, owner, annotations, interfaces, fields, methods);
            });
        }
    }
    ...
    public JavaType.@Nullable Method methodType(@Nullable MethodNode node) {
        ...
        if (node.getParameters().length > 0) {
            paramNames = new String[node.getParameters().length];
            for (int i = 0; i < node.getParameters().length; i++) {
                paramNames[i] = node.getParameters()[i].getName();
            }
        }
