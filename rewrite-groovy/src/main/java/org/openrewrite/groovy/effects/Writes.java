...
public class Writes extends org.openrewrite.java.effects.Writes implements GroovyDispatcher1<Boolean, JavaType.Variable> {

    @Override
    public Boolean visitMapEntry(G.MapEntry pp, JavaType.Variable v) {
        return writes(pp.getKey(), v) || writes(pp.getValue(), v);
    }
    ...
    public Boolean visitGString(G.GString pp, JavaType.Variable v) {
        for (J s : pp.getStrings()) {
            if (s instanceof Expression && writes(s, v)) {
                return true;
            }
        }
        return false;
