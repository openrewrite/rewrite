...
- **Heredocs live outside their node's span.** A heredoc node covers only its `<<~ID` marker. Markers are queued as they are seen and their bodies claimed the first time the cursor crosses a newline, then folded into the tree by a final pass keyed by node id.
