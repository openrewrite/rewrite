...
public class UseMatchingInsteadOfFindAll extends Recipe {
    ...
    @Getter
    final String displayName = "Use `matching(Closure)` instead of `findAll(Closure)` on Gradle container collections";
    ...
    final String description = "Gradle 9.4 deprecates the Groovy `DomainObjectCollection.findAll(Closure)` overload on " +
            ...
            "`DomainObjectCollection.matching(Closure)`, which returns a live collection that only filters " +
            "elements as they are needed by the build. This recipe only swaps the method name, leaving the " +
            "closure argument unchanged, rewriting `findAll { ... }` to `matching { ... }` when the receiver " +
            "is a known Gradle container collection.";

    /**
     * Container properties exposed by Gradle that are {@code DomainObjectCollection}s, for which the
     * Groovy {@code findAll(Closure)} overload is deprecated in favor of {@code matching(Closure)}.
