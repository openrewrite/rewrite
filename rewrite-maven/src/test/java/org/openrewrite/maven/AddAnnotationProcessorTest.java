...
class AddAnnotationProcessorTest implements RewriteTest {
    ...
    @Test
    @Issue("https://github.com/openrewrite/rewrite/issues/7384")
    void omitVersionWhenCompilerPluginVersionIsAProperty() {
        // The compiler-plugin version is provided via a property rather than inline.
        // The guard must resolve ${...} via ResolvedPom.getValue(...) before the
        // semver compare. 3.13.0 resolves from the property, so <version> is omitted.
        rewriteRun(
          pomXml(
            """
              <project>
    ...
    @Test
    @Issue("https://github.com/openrewrite/rewrite/issues/7384")
    void omitVersionWhenCompilerPluginIsManagedInSamePom() {
        // The compiler-plugin has no <version> in <build><plugins>, but the same POM's
        // <pluginManagement> supplies 3.13.0. The guard must iterate pluginManagement
        // (not just plugins) to find the effective version.
        rewriteRun(
          pomXml(
            """
              <project>
    ...
    @Test
    @Issue("https://github.com/openrewrite/rewrite/issues/7384")
    void omitVersionWhenCompilerPluginIsManagedByParentPom() {
        // Mirrors the real scenario from #7384: the child POM has no <version>
        // anywhere for maven-compiler-plugin — it's inherited from the parent's
        // <pluginManagement>. The guard must pick up the inherited management.
        rewriteRun(
          pomXml(
            """
              <project>
    ...
    @Test
    @Issue("https://github.com/openrewrite/rewrite/issues/7384")
    void keepVersionWhenCompilerPluginVersionIsUndeterminable() {
        // maven-compiler-plugin has no <version> anywhere in the effective POM — not
        // inline, not in pluginManagement, not inherited. The guard can't determine
        // the plugin version, so it falls back conservatively to keeping <version>.
        rewriteRun(
          pomXml(
            """
              <project>
