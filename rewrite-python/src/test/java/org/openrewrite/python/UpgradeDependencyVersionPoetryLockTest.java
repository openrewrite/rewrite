...
import static org.openrewrite.python.Assertions.poetryLock;
import static org.openrewrite.python.Assertions.pyproject;

/**
 * End-to-end recipe test of native poetry.lock regeneration against recorded pypi.org responses
 * (fixtures under {@code /poetrylock/}); the expected lock is byte-identical to what Poetry 2.4.1
 * wrote for the same edit.
 */
class UpgradeDependencyVersionPoetryLockTest implements RewriteTest {
    ...
    @Test
    @Timeout(120)
    void pinBumpRegeneratesPoetryLockByteForByte() {
        rewriteRun(
          spec -> spec.recipe(new UpgradeDependencyVersion("six", "==1.17.0", null, null))
            .executionContext(ctx),
          pyproject(
            resource("j-modern-upgrade/pyproject.toml.before"),
            resource("j-modern-upgrade/pyproject.toml.after"),
            s -> s.afterRecipe(doc -> {
                ...
                assertThat(marker.getPackageManager())
                        .isEqualTo(PythonResolutionResult.PackageManager.Poetry);
          ...
          poetryLock(
            resource("j-modern-upgrade/poetry.lock.before"),
            s -> s.noTrim().after(actual -> resource("j-modern-upgrade/poetry.lock.after"))
          )
        );
    }

    private static String resource(String name) {
        try (InputStream is = UpgradeDependencyVersionPoetryLockTest.class.getResourceAsStream("/poetrylock/" + name)) {
