...
@Getter
public class PythonSpaceRpcCodec extends DynamicDispatchRpcCodec<Space> {
    ...
    public void rpcSend(Space after, RpcSendQueue q) {
        new PythonSender().visitSpace(after, q);
    }

    @Override
    public Space rpcReceive(Space before, RpcReceiveQueue q) {
        return new PythonReceiver().visitSpace(before, q);
    }
}
