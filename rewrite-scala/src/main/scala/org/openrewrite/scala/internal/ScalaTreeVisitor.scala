...
class ScalaTreeVisitor(
  ...
  def visitTree(tree: Trees.Tree[?]): J = {
    tree match {
      ...
      // *before* the general `tree.isEmpty` short-circuit.
        ...
        if (sourceParents.size > 1) {
          ...
          for (i <- 1 until sourceParents.size) {
            ...
            val implType: TypeTree = visitTree(parent) match {
              case tt: TypeTree => tt
              case _ => throw unmappedException(parent)
            }
      ...
      case tb: Trees.TypeBoundsTree[?] if !tb.hi.isEmpty || !tb.lo.isEmpty =>
        ...
        if (!tb.lo.isEmpty) {
          ...
          val loType = visitTree(tb.lo) match {
            case tt: TypeTree => tt
            case _ => throw unmappedException(tb.lo)
          }
        ...
        if (!tb.hi.isEmpty) {
          ...
          val hiType = visitTree(tb.hi) match {
            case tt: TypeTree => tt
            case _ => throw unmappedException(tb.hi)
          }
    ...
    typed.tpt match {
      ...
      case _ =>
        ...
        try {
          ...
          case _: Exception => throw unmappedException(typed)
        }
    }
  }
