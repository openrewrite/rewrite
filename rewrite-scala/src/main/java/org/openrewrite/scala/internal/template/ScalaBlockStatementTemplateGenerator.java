...
public class ScalaBlockStatementTemplateGenerator extends BlockStatementTemplateGenerator {
    public ScalaBlockStatementTemplateGenerator(Set<String> imports, boolean contextSensitive) {
    ...
    protected void contextFreeTemplate(Cursor cursor, J j, Collection<JavaType.GenericTypeVariable> typeVariables, StringBuilder before, StringBuilder after) {
        if (j instanceof J.MethodInvocation) {
            ...
            throw new IllegalArgumentException(
                    ...
                    "Mark this template as context-sensitive by calling ScalaTemplate.Builder#contextSensitive().");
        } else if (j instanceof Statement && !(j instanceof J.Import) && !(j instanceof J.Package)) {
            before.insert(0, "class Template {\n");
            after.append("\n}");
        } else {
            throw new IllegalArgumentException(
                    "Scala templating is currently only implemented for context-free expressions and statements and not for `" + j.getClass() + "` instances.");
        }
