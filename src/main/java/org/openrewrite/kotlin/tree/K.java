...
public interface K extends J {
    ...
    // TODO: critique with team.
    // Infix function declarations are a Kotlin sugaring that are transformed to be more like JVM byte code during the backend process.
    // Infix functions do not need to be declared in a class, because the receiver implicitly defines how the function is called.
    // This could be a view as Jonathan stated in relation to K.Modifiers.
    @SuppressWarnings("unchecked")
    @ToString
    @FieldDefaults(makeFinal = true, level = AccessLevel.PRIVATE)
    @EqualsAndHashCode(callSuper = false, onlyExplicitlyIncluded = true)
    @AllArgsConstructor
    final class InfixFunctionDeclaration implements K, Statement, TypedTree {
        ...
        @With
        @Getter
        JRightPadded<NameTree> functionReceiver;
        ...
        @Override
        public <J2 extends Tree> J2 withMarkers(Markers markers) {
            return (J2) withMethodDeclaration(methodDeclaration.withMarkers(markers));
        }
        ...
        @Override
        public <J2 extends J> J2 withType(@Nullable JavaType type) {
            return (J2) withMethodDeclaration(methodDeclaration.withType(type));
        }
        ...
        @Override
        public <P> J acceptKotlin(KotlinVisitor<P> v, P p) {
            return v.visitInfixFunctionDeclaration(this, p);
        }
    }
