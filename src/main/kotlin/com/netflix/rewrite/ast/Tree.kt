...
sealed class Tr : Serializable, Tree {
    ...
    companion object {
         ...
         * 1-e^(-((2^26)^2)/(2*2^64)) = 0.001% for 67 million unique elements
         */
        fun id(): Long {
    ...
    data class Ident private constructor(private val ident: IdentFlyweight,
        ...
        companion object {
            ...
            fun build(name: String, type: Type? = null, formatting: Formatting = Formatting.Empty, id: Long = id()): Ident {
                val fly = flyweights
                        .getOrPut(name, { HashObjObjMaps.newMutableMap<Type?, IdentFlyweight>(mapOf(type to IdentFlyweight(name, type))) })
                        .getOrPut(type, { IdentFlyweight(name, type) })
    ...
    override fun <T: Tree> changeFormatting(fmt: Formatting): T {
        ...
        return when(this) {
            ...
            is Tr.ClassDecl.Modifier -> when(this) {
                is Tr.ClassDecl.Modifier.Public -> copy(formatting = fmt)
                is Tr.ClassDecl.Modifier.Protected -> copy(formatting = fmt)
                is Tr.ClassDecl.Modifier.Private -> copy(formatting = fmt)
                is Tr.ClassDecl.Modifier.Abstract -> copy(formatting = fmt)
                is Tr.ClassDecl.Modifier.Static -> copy(formatting = fmt)
                is Tr.ClassDecl.Modifier.Final -> copy(formatting = fmt)
            }
            ...
            is Tr.MethodDecl.Modifier -> when(this) {
                is Tr.MethodDecl.Modifier.Default -> copy(formatting = fmt)
                is Tr.MethodDecl.Modifier.Public -> copy(formatting = fmt)
                is Tr.MethodDecl.Modifier.Protected -> copy(formatting = fmt)
                is Tr.MethodDecl.Modifier.Private -> copy(formatting = fmt)
                is Tr.MethodDecl.Modifier.Abstract -> copy(formatting = fmt)
                is Tr.MethodDecl.Modifier.Static -> copy(formatting = fmt)
                is Tr.MethodDecl.Modifier.Final -> copy(formatting = fmt)
                is Tr.MethodDecl.Modifier.Synchronized -> copy(formatting = fmt)
            }
            ...
            is Tr.VariableDecls.Modifier -> when(this) {
                is Tr.VariableDecls.Modifier.Public -> copy(formatting = fmt)
                is Tr.VariableDecls.Modifier.Protected -> copy(formatting = fmt)
                is Tr.VariableDecls.Modifier.Private -> copy(formatting = fmt)
                is Tr.VariableDecls.Modifier.Abstract -> copy(formatting = fmt)
                is Tr.VariableDecls.Modifier.Static -> copy(formatting = fmt)
                is Tr.VariableDecls.Modifier.Final -> copy(formatting = fmt)
                is Tr.VariableDecls.Modifier.Transient -> copy(formatting = fmt)
                is Tr.VariableDecls.Modifier.Volatile -> copy(formatting = fmt)
            }
