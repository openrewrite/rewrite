...
public class ${lang.scopeName} internal constructor() : LanguageScope() {""")
        ...
        methods.forEach { m ->
            ...
            sb.appendLine("    public fun ${m.methodName}(action: ($nodeFqn) -> $nodeFqn) {")
            sb.appendLine("        actions[\"$key\"] = action as ((Any) -> Any)")
            sb.appendLine("    }")
            sb.appendLine()
        }
        sb.appendLine("    @Suppress(\"UNCHECKED_CAST\")")
        ...
        methods.forEach { m ->
            ...
            sb.appendLine("            val action = actions[\"$key\"] as? ((Any) -> Any)")
            sb.appendLine("            val transformed = if (action != null) action(node) as $nodeFqn else node")
