 ...
 */
interface EvalVisitorTest {
    val visitors: Iterable<EvalVisitor<*>>
        get() = emptyList()
        ...
        if (before.trimIndent() == after.trimIndent()) {
            throw RuntimeException(
                    "'before' and 'after' are equal. " +
                            ...
                            "Instead of EvalVisitorTest.assertRefactored(), use EvalVisitorTest.assertUnchanged()")
        }
    }
    ...
    fun <S : SourceFile> assertRefactored(
            parser: Parser<S>,
            visitors: Iterable<EvalVisitor<*>> = this.visitors,
            visitorsMapped: Iterable<(S) -> EvalVisitor<in S>> = emptyList(),
            visitorsMappedToMany: Iterable<(S) -> Iterable<EvalVisitor<in S>>> = emptyList(),
    ...
    fun <S : SourceFile> assertRefactored(
            parser: Parser<S>,
            visitors: Iterable<EvalVisitor<*>>,
            visitorsMapped: Iterable<(S) -> EvalVisitor<in S>>,
            visitorsMappedToMany: Iterable<(S) -> Iterable<EvalVisitor<in S>>>,
...
interface EvalVisitorTestForParser<S : SourceFile> : EvalVisitorTest {
    val parser: Parser<S>
