...
class GoBasicTypeAttributionIntegTest implements RewriteTest {
        ...
        rewriteRun(
                go(
                        """
                                ...
                                \tvar c128 complex128
                                \tvar r rune
                                \t_, _, _, _, _, _, _, _ = i, i32, b, i8, u8, f64, c128, r
                                }
                                """,
                        spec -> spec.afterRecipe(cu -> {
                            ...
                            // rune is int32 and byte is uint8, so each pair is one Go
                            // type under two spellings that attribution keeps apart.
                            assertThat(types.get("r")).isNotEqualTo(types.get("i32"));
                            assertThat(types.get("b")).isNotEqualTo(types.get("u8"));
                        })
                )
        );
    }
    ...
    /**
     * A Go int constant is 64-bit, so one above {@code Integer.MAX_VALUE} arrives
     * as {@code long}: the LST deserializer rebuilds an {@code int}-keyword
     * literal through {@code Integer.valueOf}, which throws on it.
     * Reproduces moderneinc/recipes-go#55.
     */
    @Test
    void literalsKeepAPrimitiveAcrossTheBoundary() {
        rewriteRun(
                go(
                        """
                                package main
                                ...
                                \tsmall := 42
                                \tname := "x"
                                \t_, _, _ = big, small, name
                                }
                                """,
                        spec -> spec.afterRecipe(cu -> {
                            Map<String, JavaType.Primitive> literals = literalTypes(cu);
                            assertThat(literals.get("5243700879")).isEqualTo(JavaType.Primitive.Long);
                            assertThat(literals.get("42")).isEqualTo(JavaType.Primitive.Int);
                            assertThat(literals.get("\"x\"")).isEqualTo(JavaType.Primitive.String);
                        })
                )
        );
    }

    private static Map<String, JavaType.Primitive> literalTypes(Tree cu) {
        Map<String, JavaType.Primitive> types = new LinkedHashMap<>();
        new JavaIsoVisitor<Integer>() {
            @Override
            public J.Literal visitLiteral(J.Literal literal, Integer p) {
                types.putIfAbsent(literal.getValueSource(), literal.getType());
                return literal;
            }
        }.visit(cu, 0);
        return types;
    ...
    @Test
    void builtinsAndAnyArriveIntact() {
        rewriteRun(
                go(
                        """
                                package main
                        ...
                        spec -> spec.afterRecipe(cu -> {
                            expectType(cu, "a", "any");
                            AtomicReference<J.MethodInvocation> len = new AtomicReference<>();
                            new JavaIsoVisitor<Integer>() {
                                @Override
                                public J.MethodInvocation visitMethodInvocation(J.MethodInvocation m, Integer p) {
                                    if ("len".equals(m.getSimpleName())) {
                                        len.set(m);
                                    }
                                    return super.visitMethodInvocation(m, p);
                                }
                            }.visit(cu, 0);
                            assertThat(len.get()).isNotNull();
                            assertThat(len.get().getMethodType()).isNotNull();
                            assertThat(len.get().getMethodType().getDeclaringType().getFullyQualifiedName()).isEqualTo("builtin");
                        })
                )
        );
    }
