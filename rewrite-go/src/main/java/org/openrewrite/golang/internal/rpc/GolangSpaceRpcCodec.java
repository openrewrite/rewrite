...
@Getter
public class GolangSpaceRpcCodec extends DynamicDispatchRpcCodec<Space> {
    ...
    public void rpcSend(Space after, RpcSendQueue q) {
        new GolangSender().visitSpace(after, q);
    }

    @Override
    public Space rpcReceive(Space before, RpcReceiveQueue q) {
        return new GolangReceiver().visitSpace(before, q);
    }
}
