...
func (v *GoVisitor) VisitIf(ifStmt *java.If, p any) java.J {
	...
	then := visitAndCast[*java.Block](v, ifStmt.Then, p)
	elsePart := ifStmt.ElsePart
	if elsePart != nil {
		elem := v.self().Visit(elsePart.Element, p).(java.J)
		after := v.self().VisitSpace(elsePart.After, p)
		if elem != elsePart.Element || !java.SpaceEqual(after, elsePart.After) {
			c := *elsePart
			c.Element = elem
			c.After = after
			elsePart = &c
	...
	if java.SpaceEqual(prefix, ifStmt.Prefix) && java.MarkersEqual(markers, ifStmt.Markers) &&
		condition == ifStmt.Condition && then == ifStmt.Then && elsePart == ifStmt.ElsePart {
		return ifStmt
	}
	c := *ifStmt
	c.Prefix = prefix
