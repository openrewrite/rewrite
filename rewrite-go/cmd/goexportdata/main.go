...
// checkImportPath rejects what the blob layout cannot express: a pattern
// matches several packages, and a relative path escapes the embedded
// directory and cannot be read back through an fs.FS.
...
func exportDataFor(moduleDir, importPath string) ([]byte, error) {
	...
	if blob, err := exportdata.Pack(archive); err == nil {
		return blob, nil
	}
	return archive, nil
}
